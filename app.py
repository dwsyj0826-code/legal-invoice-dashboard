...
def main():
    ...
    with st.sidebar:
        ...
        st.divider()
        with st.expander("🔍 진단 도구"):
            st.caption("특정 PDF의 실제 파싱 결과를 확인합니다.")
            if st.button("전체 스캔 진단", use_container_width=True):
                st.session_state["show_diagnostics"] = True

    # ---- 진단 모드 ----
    if st.session_state.get("show_diagnostics", False):
        st.subheader("🔍 진단 결과")
        st.caption("각 PDF에서 pdfplumber가 뽑은 실제 텍스트와 파싱 결과입니다.")
            ...
            for source, files in groups:
                ...
                for inv in invoices:
                    ...
                    if not fname.lower().endswith(".pdf"):
                        continue
                    with st.expander(f"[{firm}] {fname}"):
                        try:
                            buf = download_file(service, inv["id"])
                            with pdfplumber.open(buf) as pdf:
                                text = "\n".join((p.extract_text() or "") for p in pdf.pages)

                            pdf_month = extract_month_from_pdf_text(text)
                            name_month = extract_month(fname)
                            amount = parse_pdf_amount(text, firm)

                            c1, c2, c3 = st.columns(3)
                            c1.metric("파일명 월", str(name_month) if name_month else "실패")
